...
inspect(my.tdm)
inspect(my.dtm) # 90% sparse – meaning that 90% of its cells are either not filled with data or are zeros
...
findAssocs(my.dtm, c("medical"), 0.5)[[1]][1:5] # find co-occurrences of a word using correlation
```
...
dtm.words <- as.matrix(my.dtm[, Terms(my.dtm) %in% freq.tab$term])
map.data <- t(dtm.words) %*% dtm.words

diag(map.data) <- 0 # removing lower traingle
map.data[upper.tri(map.data)] <- NA
...
cor.data <- cor(dtm.words)
corrplot::corrplot(cor.data, method = "square", type = "lower", tl.col = "black", order = "hclust", 
